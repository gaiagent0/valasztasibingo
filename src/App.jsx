...
function formatCount(n) {
  return (COUNTER_BASE + n).toLocaleString('hu-HU');
}
...
function Modal({ isOpen, onClose, title, children, maxWidth = 480 }) {
  ...
  return (
    ...
    >
      <div style={{ minHeight: '100%', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: 8, paddingBottom: 8 }}>
        <div
          onClick={e => e.stopPropagation()}
          style={{
            background: 'white', borderRadius: 16, width: '100%', maxWidth,
            boxShadow: '0 20px 60px rgba(0,0,0,0.3)',
            animation: 'modalIn 0.22s ease', display: 'flex', flexDirection: 'column',
          }}
        >
          ...
          <div style={{ flexShrink: 0, borderRadius: '16px 16px 0 0', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.25rem 0.75rem', background: 'white' }}>
              <h2 style={{ fontSize: 17, fontWeight: 700, color: '#1a1a1a', margin: 0, lineHeight: 1.3 }}>{title}</h2>
              <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9CA3AF', padding: '4px 6px', borderRadius: 8, display: 'flex', flexShrink: 0, marginLeft: 8 }} aria-label="Bezárás">
                <X size={22} />
              </button>
            </div>
          </div>
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="🗳️ Üdvözöl a Választási Bingó!">
      <div style={{ fontSize: 14, lineHeight: 1.7, color: '#374151' }}>
        ...
        <button onClick={onClose} style={{ width: '100%', padding: '13px', background: '#CE2939', color: 'white', border: 'none', borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: 'pointer', letterSpacing: 0.5 }}>
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isBingo ? '🏆 BINGÓ! Oszd meg!' : '📤 Megosztás'}>
      <div style={{ fontSize: 14, color: '#374151' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 14 }}>
          {platforms.map(p => (
            <a key={p.name} href={p.url} target="_blank" rel="noopener noreferrer"
              style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 16px', background: p.bg, color: 'white', borderRadius: 10, textDecoration: 'none', fontWeight: 600, fontSize: 14 }}>
        ...
        <button onClick={copyText} style={{ width: '100%', padding: '11px', background: copied ? '#477050' : '#374151', color: 'white', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: 'pointer', transition: 'background 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="🔗 Beágyazás a weboldaladba">
      <div style={{ fontSize: 14, color: '#374151' }}>
        ...
        <button onClick={copyCode} style={{ width: '100%', padding: '11px', background: copied ? '#477050' : '#CE2939', color: 'white', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: 'pointer', transition: 'background 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 12 }}>
        ...
        <div style={{ background: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: 8, padding: '10px 12px', fontSize: 12, color: '#166534' }}>
          ✅ <strong>Teljesen ingyenes</strong> – hivatkozás nem szükséges, de szívesen vesszük!
        </div>
      </div>
    </Modal>
  );
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', background: '#F9FAFB', fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      <div style={{ display: 'flex', height: 7, flexShrink: 0 }}>
        <div style={{ flex: 1, background: '#CE2939' }} />
        <div style={{ flex: 1, background: '#FFFFFF', borderTop: '1px solid #E5E7EB', borderBottom: '1px solid #E5E7EB' }} />
        <div style={{ flex: 1, background: '#477050' }} />
      </div>

      <header style={{ background: 'linear-gradient(160deg, #CE2939 0%, #A01F2C 100%)', color: 'white', padding: '1.25rem 1rem 1.4rem', textAlign: 'center', position: 'relative', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 6 }}>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 800, letterSpacing: '-0.5px', textShadow: '0 2px 8px rgba(0,0,0,0.2)', margin: 0 }}>Választási Bingó 2026</h1>
          <HungarianFlag size={32} />
        </div>
      ...
      <main style={{ flex: 1, maxWidth: 520, margin: '0 auto', width: '100%', padding: '1rem 1rem 1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
          <button onClick={generateBoard}
            style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '11px 8px', background: 'white', color: '#374151', border: '1.5px solid #D1D5DB', borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}
          ...
          <button onClick={() => setShowShare(true)}
            style={{ flex: 1.4, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '11px 8px', background: '#CE2939', color: 'white', border: 'none', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: 'pointer', boxShadow: '0 3px 12px rgba(206,41,57,0.3)' }}
          ...
          <button onClick={() => setShowEmbed(true)} title="Beágyazás"
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '11px 12px', background: 'white', color: '#6B7280', border: '1.5px solid #D1D5DB', borderRadius: 10, cursor: 'pointer' }}
      ...
      <footer style={{ flexShrink: 0, textAlign: 'center', padding: '14px 16px', fontSize: 11, color: '#9CA3AF', background: '#F3F4F6', borderTop: '1px solid #E5E7EB' }}>
